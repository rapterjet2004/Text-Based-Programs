/**------------------------------------------
    ...
    Author: Julius Linus, start code provided by Dr. Sara Riazi
...
//	None	
void initField(int num_of_mine) {
	//Iterates through all possible coordinates, sets the cell to Unknown and calls setMine()
...
bool isMine(int i, int j) {
	//This checks for out-of-bounds exceptions
	if(i < 0 || j < 0)
		return false;
	else if(i > 7 || j > 4 )
		return false;

	//This checks the given cell if it is a flagged or unflagged mine
...
void reveal(int i,int j) {
	int minesCounted = 0;
    const int MARKED = 100;
    //triggers infinite loop 
    // even tho it isn't
    // try to simplify thios
    // I can make it just one single loop
    // and just mark celled that I passed instead of counting mines around it
    // I can do that later

    setCell(i, j, MARKED) = MARKED;

	//loops over every single possible coordinate pair
	for(int row=0; row<=7; row++) {
		for(int col=0; col<=4; col++) { 
			//Checks if current coordinates are within 1 unit away from given coordinates, indicating adjacency
			if(abs(row - i) <= 1 && abs(col - j) <= 1) { 
				//Checks if current coordinates are a mine, increments mine counter if true
				if(isMine(row, col))
					minesCounted++;
			}
	...
	setCell(i,j, minesCounted); //sets the cell at current coordinate equal to total minesCounted
	...
	for(int row=0; row<=7; row++) {
		for(int col=0; col<=4; col++) { 
			...
			if(abs(row - i) <= 1 && abs(col - j) <= 1) { 
				...
				// checks if adjacent coordinates are up, down, left, right from coordinate and coordinate is 0
				if(!(abs(row - i) == 1 && abs(col - j)) == 1)
				{
					if(getCell(row, col ) == UNKNOWN && getCell(i, j) == 0)
					{
						displayField(true); // remove this is for debuggingr
						reveal(row, col); 
					}
...
//	None  
void executeCmd(string cmd) {
	// Splits command string into it's individual components
	char command = cmd.at(0);
	int row = cmd.at(1)-48;
	int col = cmd.at(2)-48;

	//Evaluates the command component of the given string, commands are assumed perfect so no default case
	switch(command) {
		case 'f':
			//Checks if the given coordinates of the command are hidden (indicated by '-' in game), this is to prevent multiple flags on the same cell
			if(getCell(row,col) == UNKNOWN || getCell(row,col) == UNFLAGGED_MINE) {
				//Checks if given coordinates is a mine, sets the cell to flagged mine if true
				if(isMine(row, col)) {
					setCell(row, col, FLAGGED_MINE);

					//If flags have not run out, increments num_correct_flagged_mine to reflect the current situation
					if((num_mine - (num_correct_flagged_mine + num_incorrect_flagged_mine)) > 0)
						num_correct_flagged_mine++;
				}
				else { 
					setCell(row, col, INCORRECT_FLAGGED_MINE); //If the given coordinates is not a mine, marks cell as an incorrectly flagged mine

					//If flags have not run out, increments num_incorrect_flagged_mine to reflect the current situation
					if((num_mine - (num_correct_flagged_mine + num_incorrect_flagged_mine)) > 0)
			...
			break;
		case 'r':
			//Checks if given coordinates is a mine, explodes if true, else calls reveal
			if(isMine(row, col)) {
				explode = true;
			}
			else {
			...
			break;
		case 'u':
			//Checks if given coordinates is a mine, sets the cell to unflagged mine if true
			if(isMine(row, col)) {
				setCell(row, col, UNFLAGGED_MINE);

				//If flags have not run out, decrements num_correct_flagged_mine to reflect the current situation
				if((num_mine - (num_correct_flagged_mine + num_incorrect_flagged_mine)) > 0)
					num_correct_flagged_mine--;
			}
			else { 
				setCell(row, col, UNKNOWN); //If the given coordinates is not a mine, marks cell as an incorrectly flagged mine

				//If flags have not run out, decrements num_incorrect_flagged_mine to reflect the current situation
				if((num_mine - (num_correct_flagged_mine + num_incorrect_flagged_mine)) > 0)
					num_incorrect_flagged_mine--;
			}
			break;
